...
const passwordSchema = z.object({
    ...
    .refine((password) => /[\W_]/.test(password), 'Password must contain at least one special character'),
    confirmPassword: z.string().nonempty().min(8, 'Password must be at least 8 characters long')
    .refine((password) => /[A-Z]/.test(password), 'Password must contain at least one uppercase letter')
    .refine((password) => /[a-z]/.test(password), 'Password must contain at least one lowercase letter')
    .refine((password) => /[0-9]/.test(password), 'Password must contain at least one digit')
    .refine((password) => /[\W_]/.test(password), 'Password must contain at least one special character'),
...
const ChangePasswordForm = ({ success, passwordError } : {success: Dispatch<React.SetStateAction<boolean>>, passwordError: Dispatch<React.SetStateAction<boolean>>}) => {
    ...
    const handleSave = async (data: z.infer<typeof passwordSchema>) => {
        try {
            ...
            await axios.patch(`${backendServer}api/users/editpassword`, 
